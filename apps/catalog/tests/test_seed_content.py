...
class SeedContentTests(TestCase):
    ...
    def test_credits_linked_with_roles_and_characters(self):
        ...
        lead = title.participations.get(person__slug="dzhessi-ayzenberg")
        self.assertEqual(lead.character, "Марк Цукерберг")

    def test_person_has_filmography_across_titles(self):
        """Смысл единой модели персоны: один актёр — во многих фильмах."""
        fincher = Person.objects.get(slug="devid-fincher")
        self.assertGreaterEqual(fincher.titles.count(), 1)

    def test_collections_published_with_items(self):
        self.assertEqual(Collection.objects.published().count(), len(COLLECTIONS))

        collection = Collection.objects.get(slug="istorii-kotorye-izmenili-mir")
    ...
    def test_reviews_create_real_ratings(self):
        ...
        expected = len(REVIEWS["sotsialnaya-set-2010"])
    ...
    def test_related_titles_set(self):
        title = Title.objects.get(slug="sotsialnaya-set-2010")
        self.assertEqual(title.related_titles.count(), 0)
