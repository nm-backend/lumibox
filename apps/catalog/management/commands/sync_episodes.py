...
Тянет GET /serials/kp/{id} (или /serials/imdb/{id}) для записей каталога
с kp_id/imdb_id, у которых нет ни одной серии, и создаёт недостающие
серии. Повторный запуск ничего не дублирует: пары «сезон + серия»
уникальны в пределах записи.

Эндпоинт описан в документации сервиса, но на момент проверки
(август 2026) отдаёт 404 на любом ID — команда к работе готова и
начнёт наполнять серии, когда сервис включит эндпоинт; сейчас она
честно сообщит, что записей не нашлось.
