...
Для запуска Vibix у демонстрационного фильма достаточно применить миграции:
«Начало» получит реальный `kp_id=447301`, а страница сформирует тег плеера
с вашим `VIBIX_PUBLISHER_ID`. После добавления `VIBIX_API_TOKEN` запустите
в shell веб-сервиса:
