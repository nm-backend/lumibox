...
**Просмотр**
- Источники видео в одной модели `PlaybackSource`: свой файл или внешний
  плеер, у сериала — на каждой серии
- Выбор озвучки, если их несколько; выбор сохраняется между сериями
- Серии сгруппированы по сезонам
- Продолжение просмотра с той же секунды, а не с начала серии
- Внешние плееры пускаются во фрейм только с доверенных хостов
- Внешний плеер по ID Кинопоиска/IMDb или внутреннему ID видео: вкладка
  появляется у записей с заполненными ID, а ID проставляет синхронизация
  каталога
...
## Интеграция с внешним видеосервисом

Каталог подключается к внешнему видеосервису: записи получают
Kinopoisk/IMDb-идентификаторы и внутренний ID видео, а страница тайтла
показывает вкладку с плеером, который сам находит видео по этим ID.
...
плеер получает `data-trailer="true"` — покажет трейлер, если полного
видео нет в каталоге. SDK и тег `<ins>` подключаются только на таких
страницах. Пустой `VIDEO_SERVICE_PUBLISHER_ID` в настройках отключает
интеграцию целиком.
...
не затираются. Первый прогон фильтрует каталог по годам записей
и занимает минуты, дальше идёт по отметке `updated_from` и подхватывает
только новые/изменённые видео.
...
**527 тестов** (3 отмечены skip), около минуты. Ожидаемый результат — `OK`.
